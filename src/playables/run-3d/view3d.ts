...
import { RUN, TRACK, TRACK_LEN, GATE_COLOR, COPY, Gate, Row, Finish, opLabel, opGood } from './config';
import { State } from './state';
import { Layout, UiState } from './layout';
import { Hud } from './hud';
import { Fx } from '../../core/fx';
import { Crowd, blobTexture } from './crowd';
...
/** Duvarın istediği sayı — bitişten çok önce okunabilmeli. */
function needTexture(need: number): CanvasTexture {
  const cv = document.createElement('canvas');
  cv.width = 256;
  cv.height = 128;
  const g = cv.getContext('2d') as CanvasRenderingContext2D;
  g.fillStyle = 'rgba(28,40,36,.9)';
  ...
  g.textBaseline = 'top';
  g.fillText(COPY.need, 128, 12);
  g.fillStyle = '#FFD45F';
  g.font = '900 76px system-ui,-apple-system,Segoe UI,Roboto,sans-serif';
  g.textBaseline = 'middle';
  g.fillText(String(need), 128, 82);
  return new CanvasTexture(cv);
}
...
export class View3D implements RunView {
   ...
   */
  private hazard = new MeshBasicMaterial({ color: new Color('#E23B33') });
  private hazardScar = new MeshBasicMaterial({
    color: new Color('#5E2A18'),
    transparent: true,
    opacity: 0.22,
  });
  ...
  constructor(gl: HTMLCanvasElement) {
    ...
    this.crowd = new Crowd(CHAR_H, 'sprint');
  ...
  private buildGround(): void {
    ...
    const path = new Mesh(
      new PlaneGeometry(RUN.halfW * 2 + 0.5, len),
    ...
    for (const d of [-1, 1]) {
      ...
      rim.position.set(d * (RUN.halfW + 0.35), 0.06, len / 2 - 30);
      this.scene.add(rim);
    }
  }
  ...
  private buildScenery(): void {
    ...
    for (let z = -16; z < TRACK_LEN + 60; z += 5.4) {
      for (const d of [-1, 1]) {
        ...
        if (!g) continue;
        const px = d * (RUN.halfW + 1.5 + rnd() * 5);
        const pz = z + rnd() * 3;
      ...
      for (const d of [-1, 1]) {
        ...
        if (!far) continue;
        far.position.set(d * (RUN.halfW + 7 + rnd() * 8), 0, z + rnd() * 5);
      ...
      for (const d of [-1, 1]) {
        for (let k = 0; k < 3; k++) {
          ...
          if (!tiny) continue;
          tiny.position.set(d * (RUN.halfW + 0.5 + rnd() * 0.9), 0, z + rnd() * 5.2);
      ...
      if (rnd() < 0.24) {
        for (const d of [-1, 1]) {
          for (let k = 0; k < 3; k++) {
            ...
            if (!f) continue;
            f.position.set(d * (RUN.halfW + 0.75), 0, z + k * 1.0);
  ...
  private buildTrack(): void {
    for (const ev of TRACK) {
      ...
      else if (ev.type === 'row') this.buildRow(ev);
      else this.buildFinish(ev);
    }
  }
  ...
  private buildRow(ev: Row): void {
    ...
    for (const b of ev.blocks) {
      ...
      //
      // İlk sürüm parçayı sadece boya göre ölçekliyordu ve engel olarak
      // seçilen yayvan kaya, kapatması gereken aralığın iki katı yer
      // kaplıyordu — ekranda boşluk kapanmış gibi duruyordu. Artık boy
      // hedefi ölçeği veriyor, en ise aralığa tam oturacak şekilde
      // ayrıca sıkıştırılıyor.
      const k = size ? H / (size.y || 1) : 1;
      const wEach = size ? size.x * k : 1;
      const n = Math.max(1, Math.round(span / Math.max(0.6, wEach)));
      for (let i = 0; i < n; i++) {
        const p = propClone(ev.prop, H);
        if (!p) continue;
        if (size) p.scale.x = span / n / (size.x || 1);
        p.position.set(LANE * (b[0] + (span * (i + 0.5)) / n), 0, 0);
        g.add(p);
      }
      // Zemindeki uyarı bandı: engelin nerede BİTTİĞİ, yani boşluğun nerede
      // olduğu, modellerin siluetinden daha hızlı okunuyor. Engelin ÖNÜNDE
      // duruyor; altına konunca modeller örtüyor ve hiç görünmüyordu.
      const cx = (LANE * (b[0] + b[1])) / 2;
      const scar = new Mesh(new PlaneGeometry(span, 2.2), this.hazardScar);
      scar.rotation.x = -Math.PI / 2;
      scar.position.set(cx, 0.03, -1.2);
      g.add(scar);
      const line = new Mesh(new PlaneGeometry(span, 0.26), this.hazard);
      line.rotation.x = -Math.PI / 2;
      line.position.set(cx, 0.04, -2.3);
      g.add(line);
    }
    g.position.z = ev.z;
    this.scene.add(g);
  ...
  /**
   * SAHNEYİ MALZEMEYE GÖRE BİRLEŞTİR.
   *
   * Koridorun kenarında ~55 ağaç, kaya ve çit var; her biri 2-3 malzemeli,
   * yani 2-3 ayrı çizim çağrısı. Ölçtük: sahne 337 çizim çağrısıyla açılıyordu
   * ve bunun 150'den fazlası sadece süstü. Hepsi hiç kımıldamadığına göre
   * ayrı nesne olmalarının bir sebebi yok: aynı malzemeyi paylaşan bütün
   * yüzeyler dünya koordinatına taşınıp TEK bir mesh'te toplanıyor.
   *
   * Vertex verisi burada float'a çevriliyor. Modeller quantize edilmiş
   * (int16) geliyor ve ölçek/kaydırma düğüm dönüşümünde duruyor; birleştirme
   * o dönüşümü vertex'e pişirdiği için tip de değişmek zorunda.
   */
  ...
  private buildFinish(ev: Finish): void {
    const cols = 7;
    const rows = 3;
    const bw = (RUN.halfW * 2) / cols;
    const bh = 0.92;
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        b.position.set(-RUN.halfW + bw * (c + 0.5), r * bh, 0);
    ...
    const sign = new Mesh(
      ...
      new MeshBasicMaterial({ map: needTexture(ev.need), transparent: true, side: DoubleSide })
    );
    // Kapı panelleriyle aynı sebep: ön yüzü kameraya çevir, yoksa yazı
    // arkadan, yani ayna görüntüsü olarak görünüyor.
    sign.rotation.y = Math.PI;
    sign.position.set(0, rows * bh + 0.95, -0.05);
    this.wall.add(sign);
    this.wall.position.z = ev.z;
  ...
  private crowdScreen(cx: number, cz: number): [number, number] {
  ...
  gate(good: boolean, label: string): void {
    const [x, y] = this.crowdScreen(this.lastX, this.lastZ);
  ...
  finish(won: boolean): void {
    // Kazanınca klip DEĞİŞMİYOR: kalabalık koşarak deliğin içinden geçiyor.
    // Sevinme animasyonu koşuyla çelişiyordu — aynı anda hem koş hem zıpla.
    if (won) this.smashT = 0;
    else this.crowd.setClip('idle');
  }
