 ...
 */
import { RUN, TRACK, TRACK_LEN, COPY } from './config';
...
export class Hud {
  ...
  reset(): void {
    this.shown = RUN.start;
    this.lastCount = RUN.start;
    this.punch = 0;
    this.pops.length = 0;
  }
  ...
  draw(s: State, ui: UiState, dt: number): void {
    ...
    if (s.count !== this.lastCount) {
      this.punchUp = s.count > this.lastCount;
      this.lastCount = s.count;
      this.punch = 1;
    }
    this.punch = Math.max(0, this.punch - dt * 2.6);
    this.shown += (s.count - this.shown) * (1 - Math.exp(-9 * dt));
    if (Math.abs(s.count - this.shown) < 0.02) this.shown = s.count;
  ...
  private progress(s: State, y: number): void {
    ...
    for (const ev of TRACK) {
      ...
      const done = s.z >= ev.z;
      g.fillStyle = ev.type === 'finish'
        ? '#E5484D'
        : done ? 'rgba(255,255,255,.85)' : 'rgba(40,60,55,.42)';
      const r = ev.type === 'finish' ? h * 0.95 : h * 0.62;
      g.beginPath();
  ...
  private endcard(s: State): void {
    ...
    const et = s.endT - RUN.endAfter;
    const celebrating = et < RUN.celebrateFor;
    ...
    fitFont(g, String(s.count), L.w * 0.5, '900', numSize, FONT);
    outlinedText(g, String(s.count), L.w / 2, cy + size * 1.06, numSize,
    ...
    g.fillText(won ? s.count + ' ' + COPY.winSub : COPY.loseSub,
      L.w / 2, cy + size * 1.06 + Math.min(L.w * 0.19, 92));

    if (celebrating) return;

    const fade = Math.min(1, (et - RUN.celebrateFor) / 0.3);
    g.save();
    g.globalAlpha = fade;
    const by = L.h * 0.63;
